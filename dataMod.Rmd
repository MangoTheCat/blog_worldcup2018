...
Next we load three datasets that have been tidied up from open source resource or updated from original version. Plenty of time was spent on gathering data, aligning team names and cleaning up features (surprise huh). 
...
group_match_data <- group_match_data %>% left_join(select(team_data, number, name), by = c("team1" = "name")) %>% rename(number1 = number) %>% left_join(select(team_data, number, name), by = c("team2" = "name")) %>% rename(number2 = number)
...
play_game(teams = team_data, play_fun = "play_fun_skellam", 
          team1 = 7, team2 = 8, 
          musthavewinner=FALSE, normalgoals = normalgoals)

play_game(teams = team_data, play_fun = "play_fun_elo", 
          team1 = 7, team2 = 8)

play_game(teams = team_data, train_data = wcmatches_train, 
...
Training set **wcmatches_train** has a **home** column, representing whether team X in match Y is home team. However, it's hard to say in a third country a team/away position makes much difference comparing to league competetions. Also I didn't find an explicit home/away split for Russian World Cup. We can derive a similar  like variable indicating host nation or continent in future model interation. Home advantage stands no better chance for the time being.  


# Group and kickout stages

Find the winners at various stages, from group to round of 16, quarter-finals, semi-finals and final. It's how the game is  They are executed in each tournament simulation. 

```{r find_winner}
# Set seed to get same result
devtools::load_all("worldcup")
set.seed(1984)
find_group_winners(teams = team_data, 
                   group_match_data = group_match_data, 
                   play_fun = "play_fun_simplest",
                   normalgoals = 3)

set.seed(1984)
simulate_one(play_fun = "play_fun_simplest",        # play_fun,
             teams = team_data,                     # team_data, 
             group_matches = group_match_data, 
             normalgoals = 5) 
set.seed(1984)
x1 = simulate_tournament(nsim = 100,
                    play_fun = "play_fun_simplest",
                    teams = team_data, 
                    group_matches = group_match_data, 
                    normalgoals = 1)

x5 = simulate_tournament(nsim = 100,
                    play_fun = "play_fun_simplest",
                    teams = team_data, 
                    group_matches = group_match_data, 
                    normalgoals = 5)

get_top_scorer(nsim = 100, result_data = x1)
get_top_scorer(nsim = 100, result_data = x5)
#-----------------------------------------
set.seed(1984)
find_group_winners(teams = team_data, 
                   group_match_data = group_match_data, 
                   ...
                   normalgoals = 2.75)
find_knockout_winners(teams = team_data, 
                      match_data = structure(c(3L, 8L, 10L, 13L), .Dim = c(2L, 2L)), 
                      play_fun = "play_fun_skellam",
                      normalgoals = 2.75)
simulate_one(play_fun = "play_fun_skellam",        # play_fun,
             teams = team_data,                     # team_data, 
             group_matches = group_match_data, 
             normalgoals = 2.75) 
...
set.seed(1984)
result <- simulate_tournament(nsim = nsim, play_fun = "play_fun_simplest", normalgoals = normalgoals) 
...
`get_winner()` reports a winner list who has highest probability. My gut feeling probably disagrees with the random poison result but can't really tell which one is the best out of the other three models. 
